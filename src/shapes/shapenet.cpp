...
public:
    ...
    void checkTriangle(ShapeNetTriangle& tri, ShapeNetTriangle& t) {
    ...
    size_t hash(const ShapeNetTriangle& t) {
        size_t h0 = std::hash<int>{}(t.p[0]);
        size_t h1 = std::hash<int>{}(t.p[1]);
        size_t h2 = std::hash<int>{}(t.p[2]);
        return h0 ^ h1 ^ h2;
    }

    bool findTriangle(ShapeNetTriangle& t, std::unordered_map<int, std::vector<ShapeNetTriangle*> > hashmap, ShapeNetTriangle** found) {
        size_t h = hash(t);
        if (hashmap.find(h) == hashmap.end()) return false;
        auto tris = hashmap[h];
        for (auto tri : tris) {
            if (*tri == t) {
                *found = tri;
    ...
    ShapeNetOBJ(const Properties &props) : Shape(props) {
        ...
        std::unordered_map<int, std::vector<ShapeNetTriangle*>> hashmap;
        ...
        while (is.good() && !is.eof() && fetch_line(is, line)) {
            ...
            if (buf == "v") {
                ...
                if (!props.hasProperty("bsdf") && !materialLibrary.empty())
                ...
                ShapeNetTriangle* existing = NULL;
                if (findTriangle(t, hashmap, &existing)) {
                    checkTriangle(*existing, t);
                } else {
                    triangles.push_back(t);
                    size_t h = hash(t);
                    if (hashmap.find(h) == hashmap.end()) {
                        hashmap[h] = std::vector<ShapeNetTriangle*>();
                    }
                ...
                while (iss >> tmp) {
                    ...
                    if (findTriangle(t, hashmap, &existing)) {
                        ...
                        size_t h = hash(t);
                        if (hashmap.find(h) == hashmap.end()) {
                            hashmap[h] = std::vector<ShapeNetTriangle*>(1);
                        }
                        hashmap[h].push_back(&t);
                    }
                }
        ...
        Log(EInfo, "Done parsing \"%s\" .", path.filename().string().c_str());
    ...
    void loadMaterialLibrary(const FileResolver *fileResolver, const fs::path &mtlPath) {
        ...
        for (std::map<std::string, Texture *>::iterator it = cache.begin(); it != cache.end(); ++it)
            it->second->decRef();
        Log(EInfo, "Done loading OBJ materials from \"%s\" ..", mtlPath.filename().string().c_str());
    }
    ...
    void createMesh0(const std::string& targetName,
        ...
        bool faceNormal)
    {
        Log(EInfo, "Grouping by material...");
        TriGroup group = groupTriByMtl(triangles);
        Log(EInfo, "Done grouping by material.");
        ...
        for (auto it1 = group.begin(); it1 != group.end(); ++it1)
        {
            ...
            for (auto it2 = it1->second.begin(); it2 != it1->second.end(); ++it2)
            {
                ...
                if (!!bsdf1 && bsdf1->hasComponent(BSDF::ETransmission))
                {
                    bsdf = bsdf1;
                }
                else if (!!bsdf2 && bsdf2->hasComponent(BSDF::ETransmission))
                {
                    bsdf = bsdf2;
                }
